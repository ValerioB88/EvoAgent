...
from evoenv.model import Environment, FoodToken
from evoenv.evoagent import EvoAgent
...
class SimpleCanvas(VisualizationElement):
    local_includes = ["code/evoenv/simple_continuous_canvas.js"]
        ...
        for idx, obj in enumerate(agents):
            ...
            if model.selected_agent_unique_id == obj.unique_id:
                ...
                if not obj.network_drawn:
                    vis.draw_net(model.config, obj.genome, view=False, filename= f'./data/sim/nets/net_{obj.unique_id}')
                ...
                portrayal['children_pos'] = [list(agents[find_pop_idx_from_id(agents, i)].pos) for i in  obj.children_id]
                portrayal['pos'] = str(obj.pos.astype(int))
                portrayal['print_info'] =['NAME', 'pop_idx', 'generation', 'unique_id',  'age', 'parent_id', 'num_children', 'children_id', 'energy', 'countdown_offspring', 'pos', 'skin_c']
